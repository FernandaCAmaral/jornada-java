...
| 02 | [🎵 AudioPlayer](./02-desafio-audio-player)              | Orientação a Objetos (Herança, Encapsulamento, Polimorfismo) e Pacotes. | Testes lógicos baseados em regras de negócio específicas. |
...
| 04 | [📡 Buscador de CEP](./04-desafio-buscador-cep)          | Integração com APIs, Biblioteca Gson, `Java Records`, Escrita em arquivos físicos (`FileWriter`). | Blindagem contra instabilidade de rede, tratamento de exceções (`try-catch`), validação de formatos de dados e tratamento de respostas nulas/inválidas da API. |
